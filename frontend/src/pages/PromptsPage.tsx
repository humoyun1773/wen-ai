        ...
        <div className="max-w-5xl mx-auto w-full space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-surface-border/60 pb-6">
            <div>
              <div className="flex items-center gap-2">
                <Terminal className="w-6 h-6 text-primary-light" />
                <h1 className="text-2xl font-bold text-white tracking-tight">
                  System Prompts & AI Personalar
                </h1>
              </div>
              <p className="text-xs text-zinc-400 mt-1">
                AI ga maxsus rollar, ko'rsatmalar va xulq-atvor qoidalarini biriktiring.
              </p>
            </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-400">
              Tayyor Shabonlar (1-bosishda qo'shish)
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {PRESET_TEMPLATES.map((tmpl, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-2xl bg-surface border border-surface-border hover:border-primary/40 transition-all flex items-start justify-between gap-3"
                >
                  <div className="space-y-1 min-w-0">
                    <h4 className="text-xs font-bold text-white truncate">
                      {tmpl.name}
                    </h4>
                    <p className="text-[11px] text-zinc-400">{tmpl.description}</p>
                  </div>
                  <Button
                    size="sm"
                    variant="ghost"
                    className="text-primary-light hover:bg-primary/10 text-xs flex-shrink-0"
          ...
          <div className="space-y-4 pt-4">
            <h2 className="text-base font-semibold text-white">
              Sizning Promplaringiz ({prompts.length})
            </h2>
                ...
                {prompts.map((p) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-primary/10 text-primary-light border border-primary/20">
                          {p.category}
                        </span>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-surface-light"
                          title="Tahrirlash"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-zinc-400 hover:text-red-400 hover:bg-surface-light"
                          title="O'chirish"
                        >
                    ...
                    <div className="p-3 bg-surface-dark rounded-xl border border-surface-border/50 text-xs text-zinc-300 font-mono line-clamp-3">
                      {p.content}
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full bg-surface-dark border border-surface-border rounded-xl p-3 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-primary resize-none font-mono"
              />
            </div>
